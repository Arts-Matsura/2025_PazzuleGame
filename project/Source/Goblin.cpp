...
Goblin::Goblin()
{
	static const std::string folder = "data/Character/Goblin/";
	hModel = ResourceLoader::MV1LoadModel((folder + "Goblin.mv1").c_str());
	assert(hModel > 0);
	...
	animator->LoadAnimFile(folder + "Anim_Damage.mv1", false);
...
void Goblin::Update()
{
	...
	if (animator->PlayingID() == aWALK) {
		...
		if (frame < 1.0f) {//ここで足音を鳴らす
			hFootSE = ResourceLoader::LoadSoundMem("data/Sound/SE/Goblin/MAT_footstep_grass_00.wav");
...
bool Goblin::CollisionAttacked(VECTOR n1, VECTOR n2, VECTOR p1, VECTOR p2)
{
	if (state == sDead)
		return false;
...
void Goblin::updateErase()
{
}

void Goblin::moveToTarget(VECTOR target)
{
	// ここで角度を合わせる 
